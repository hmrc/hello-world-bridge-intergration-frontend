...
import forms.FindAPropertyForm.form
...
extends FrontendController(mcc) with I18nSupport {
  ...
  def onPageLoad: Action[AnyContent] =
    identify.async { implicit request =>
      Future.successful(Ok(findAPropertyView(form)))
    }
  ...
  def onSubmit: Action[AnyContent] =
    identify.async { implicit request =>
      form.bindFromRequest().fold(
        formWithErrors =>
          Future.successful(BadRequest(findAPropertyView(formWithErrors))),

        findAProperty => {
          connector.findAPropertyPostcodeSearch(findAProperty).flatMap {
            case Left(error) =>
