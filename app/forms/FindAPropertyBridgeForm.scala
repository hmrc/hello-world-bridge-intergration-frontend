...
case class FindAPropertyBridgeForm(postcode: Postcode, propertyName: Option[String]) {
  override def toString: String = Seq(propertyName, postcode.value).mkString(",")
}

object FindAPropertyBridgeForm extends Constraints {
  implicit val format: OFormat[FindAPropertyBridgeForm] = Json.format[FindAPropertyBridgeForm]
  private val postcode = "postcode-value"
  private val propertyName = "property-name-value"

  def unapply(findAProperty: FindAPropertyBridgeForm): Option[(Postcode, Option[String])] = Some((findAProperty.postcode, findAProperty.propertyName))

  def form: Form[FindAPropertyBridgeForm] =
    Form(
      mapping(
        postcode -> text()
      ...
      )(FindAPropertyBridgeForm.apply)(FindAPropertyBridgeForm.unapply)
    )
}
