...
class CheckAnswersSummaryListRowSpec
  extends AnyWordSpec
    with Matchers
    with OptionValues {
  ...
  "CheckAnswersSummaryListRow.summarise" should {
    "render a key with title and caption when captionKey is defined" in {
      val row = CheckAnswersSummaryListRow(
      ...
      val result = CheckAnswersSummaryListRow.summarise(row)
      
      val html =
        result.key.content.asInstanceOf[HtmlContent].value.toString
    ...
    "render multiple values joined with <br> and an id derived from the title" in {
      ...
      val html =
        result.value.content.asInstanceOf[HtmlContent].value.toString
