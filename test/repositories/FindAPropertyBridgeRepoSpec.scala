...
import models.bridge.search.{PostcodeSearchResult, Results}
...
class FindAPropertyBridgeRepoSpec
  extends AnyWordSpec
    with Matchers
    with ScalaFutures
    with MockitoSugar {
  ...
  private val mockCollection =
    mock[MongoCollection[StoredPostcodeSearchResults]]

  private val repo = new FindAPropertyBridgeRepo(mongoComponent, config) {

    override lazy val collection: MongoCollection[StoredPostcodeSearchResults] =
      mockCollection
  ...
  "FindAPropertyBridgeRepo.upsert" should {
    ...
    "return true when Mongo acknowledges the upsert" in {
      ...
      when(
        mockCollection.replaceOne(
          any[Bson],
          any[StoredPostcodeSearchResults],
          any[ReplaceOptions]
        )
      ).thenReturn(SingleObservable(updateResult)) // ✅ FIX

      repo.upsert("user-1", PostcodeSearchResult(Results(1, 1, Nil))).futureValue mustBe true
    }
    ...
    "fail with IllegalStateException when Mongo fails" in {
      ...
      val ex =
        repo.upsert("user-1", PostcodeSearchResult(Results(1, 1, Nil))).failed.futureValue

      ex mustBe a[IllegalStateException]
      ex.getMessage must include("Stored properties have not been inserted: mongo down null")
    }
  }
}